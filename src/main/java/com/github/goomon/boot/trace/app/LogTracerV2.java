...
public class LogTracerV2 {
    ...
    public TraceStatus beginSync(TraceId prevTraceId, String message) {
        TraceId curTraceId = prevTraceId.createNextId();
        Long startTimeMs = System.currentTimeMillis();
        log.info("[{}] {}{}", curTraceId.getId(), addSpace(START_PREFIX, curTraceId.getLevel()), message);
        return new TraceStatus(curTraceId, startTimeMs, message);
    }
